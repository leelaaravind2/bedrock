/*
 * Thraksha — Day 16 EXECUTION gate harness (temporary; not part of the product).
 *
 * Drives the REAL UI server (server.ts) over HTTP exactly as the browser does,
 * proving the projectType wizard wiring is byte-for-byte the engine:
 *
 *   WEB-APP   A fresh untouched wizard (projectType='Web App') reproduces all 20
 *             default hashes — and UI == CLI for each. Web-App is a literal bypass.
 *   API-ONLY  UI == CLI for the type: a wizard-chosen projectType='API-only'
 *             (submitting frontend='React' exactly as the wizard's setSettings now
 *             does → the model normalises to None AND records the ADR-004 default)
 *             is BYTE-IDENTICAL to the Day-15 baselines, across all five stacks
 *             (Spring subtracts its frontend; the four backend-only stacks differ
 *             from Web-App only in the two manifest lines + the recorded default).
 *
 * Run:  node dist/day16-gate.js
 */
...
// Day-15 api-only baselines (the recorded-default variant: frontend submitted
// non-None → model normalises to None AND records the ADR-004 default). Postgres.
...
// The exact body setSettings() sends. For API-only the wizard submits
// frontend='React' (Step 1) so the model normalises to None AND records the
// ADR-004 default — the canonical Day-15 baseline. Web-App also uses 'React'
// (the frozen 20-hash baselines use frontend='React'), so both are 'React' here.
function settingsBody(projectName: string, projectType: string, backend: string, database: string) {
  return { projectName, projectType, backend, frontend: 'React', database, multiUser: true, auth: 'Simple login' };
}
...
/** CLI content hash (leading-slash), matching hashTree byte-for-byte. */
...
async function main(): Promise<void> {
  const root = path.join(os.tmpdir(), 'thraksha-day16-gate');
  ...
  async function wizardHash(projectName: string, projectType: string, backend: string, database: string, entities: unknown[]): Promise<string> {
    const dir = path.join(outputRoot, projectName);
    await fs.rm(dir, { recursive: true, force: true, maxRetries: 10, retryDelay: 100 });
    await api('POST', '/api/settings', settingsBody(projectName, projectType, backend, database));
  ...
  // ── WEB-APP — fresh untouched wizard reproduces the 20; UI == CLI ───────────
  process.stdout.write('=== WEB-APP: fresh wizard == 20 default hashes, UI==CLI ===\n');
  for (const model of ['DemoApp', 'TeamTracker']) {
    for (const db of DATABASES) {
      for (const backend of BACKENDS) {
        const key = `${backend}|${db}|${model}`;
        const entities = model === 'DemoApp' ? [TICKET] : TEAMTRACKER;
        const ui = await wizardHash(model, 'Web App', backend, db, entities);
        const cli = await cliHash(model === 'DemoApp' ? buildDemoAppModel({ backend, database: db }) : buildTeamTrackerModel({ backend, database: db }));
        const ok = ui === FROZEN[key] && ui === cli;
        if (!ok) pass = false;
        process.stdout.write(`  ${ok ? 'OK  ' : 'FAIL'} ${key.padEnd(34)} ${ui.slice(0, 16)} frozen=${ui === FROZEN[key]} UI==CLI=${ui === cli}\n`);
      }
    }
  }

  // ── API-ONLY — UI == CLI across all five stacks (== Day-15 baselines) ───────
  process.stdout.write('\n=== API-ONLY: wizard == Day-15 baselines, all five stacks (UI==CLI) ===\n');
  for (const backend of BACKENDS) {
    const models = backend === 'Spring Boot' ? ['DemoApp', 'TeamTracker'] : ['DemoApp'];
    for (const model of models) {
      const key = `${backend}|${model}`;
      const entities = model === 'DemoApp' ? [TICKET] : TEAMTRACKER;
      const ui = await wizardHash(model, 'API-only', backend, 'PostgreSQL', entities);
      const cli = await cliHash(model === 'DemoApp' ? buildDemoAppModel({ backend, database: 'PostgreSQL', projectType: 'API-only' }) : buildTeamTrackerModel({ backend, database: 'PostgreSQL', projectType: 'API-only' }));
      const okBase = ui === API_ONLY[key];
      const okUiCli = ui === cli;
      if (!okBase || !okUiCli) pass = false;
      process.stdout.write(`  ${okBase && okUiCli ? 'OK  ' : 'FAIL'} ${key.padEnd(28)} ${ui.slice(0, 16)} baseline=${okBase} UI==CLI=${okUiCli}\n`);
    }
  }

  await fs.rm(root, { recursive: true, force: true, maxRetries: 10, retryDelay: 200 });
  process.stdout.write(`\nDay-16 gate: ${pass ? 'PASS' : 'FAIL'}\n`);
